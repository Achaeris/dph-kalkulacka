import tkinter as tk
from tkinter import messagebox

APP_NAME = "DPH kalkulačka (12 % / 21 %)"
APP_VERSION = "1.0"
AUTHOR = "Dariusz Zabdyr (Darek)"

def parse_amount(text: str) -> float:
    # Podpora čárky i tečky + odstranění mezer
    t = text.strip().replace(" ", "").replace("\u00A0", "").replace(",", ".")
    return float(t)

def vypocitej():
    try:
        zaklad = parse_amount(entry_zaklad.get())
        if zaklad < 0:
            raise ValueError

        sazba = float(var_sazba.get())  # 0.12 nebo 0.21
        dph = zaklad * sazba
        celkem = zaklad + dph

        label_vysledek.config(
            text=(
                f"Sazba: {int(sazba*100)} %\n"
                f"Základ: {zaklad:,.2f} Kč\n"
                f"DPH: {dph:,.2f} Kč\n"
                f"Celkem: {celkem:,.2f} Kč"
            ).replace(",", " ")
        )

        # Do schránky dáme "řádkový" formát
        root.clipboard_clear()
        root.clipboard_append(
            f"Základ {zaklad:.2f} Kč | DPH {int(sazba*100)}% {dph:.2f} Kč | Celkem {celkem:.2f} Kč"
        )

    except Exception:
        messagebox.showerror("Chyba", "Zadej prosím platné číslo (může být i s čárkou).")

def vycisti():
    entry_zaklad.delete(0, tk.END)
    label_vysledek.config(text="")
    entry_zaklad.focus_set()

def kopiruj():
    txt = label_vysledek.cget("text").strip()
    if not txt:
        messagebox.showinfo("Info", "Nejdřív něco vypočítej 🙂")
        return
    root.clipboard_clear()
    root.clipboard_append(txt.replace("\n", " | "))
    messagebox.showinfo("Zkopírováno", "Výsledek je v schránce.")

def show_help():
    messagebox.showinfo(
        "Pomoc",
        "Jak používat:\n"
        "1) Zadej částku bez DPH (základ).\n"
        "2) Vyber sazbu 12 % nebo 21 %.\n"
        "3) Klikni na Vypočítat (nebo stiskni Enter).\n\n"
        "Tipy:\n"
        "• Můžeš psát i s čárkou (např. 123,45).\n"
        "• Kopírovat dá výsledek do schránky."
    )

def about_app():
    messagebox.showinfo(
        "O aplikaci",
        f"{APP_NAME}\nVerze: {APP_VERSION}\n\nJednoduchý výpočet DPH ze základu."
    )

def about_author():
    messagebox.showinfo(
        "O autorovi",
        f"Autor: {AUTHOR}\n"
        "Licence: GNU GPL v2\n\n"
        "Vytvořeno v Pythonu (Tkinter)."
    )

root = tk.Tk()
root.title(APP_NAME)
root.resizable(False, False)
root.geometry("380x280")

# Menu
menubar = tk.Menu(root)
help_menu = tk.Menu(menubar, tearoff=0)
help_menu.add_command(label="Pomoc", command=show_help)
help_menu.add_separator()
help_menu.add_command(label="O aplikaci", command=about_app)
help_menu.add_command(label="O autorovi", command=about_author)
menubar.add_cascade(label="Nápověda", menu=help_menu)
root.config(menu=menubar)

frame = tk.Frame(root, padx=12, pady=12)
frame.pack(fill="both", expand=True)

tk.Label(frame, text="Částka bez DPH (základ):").grid(row=0, column=0, sticky="w")
entry_zaklad = tk.Entry(frame, width=25)
entry_zaklad.grid(row=1, column=0, sticky="w", pady=(4, 12))
entry_zaklad.focus_set()

tk.Label(frame, text="Sazba DPH:").grid(row=2, column=0, sticky="w")
var_sazba = tk.StringVar(value="0.21")

radios = tk.Frame(frame)
radios.grid(row=3, column=0, sticky="w", pady=(4, 12))
tk.Radiobutton(radios, text="12 %", variable=var_sazba, value="0.12").pack(side="left", padx=(0, 12))
tk.Radiobutton(radios, text="21 %", variable=var_sazba, value="0.21").pack(side="left")

btns = tk.Frame(frame)
btns.grid(row=4, column=0, sticky="w")
tk.Button(btns, text="Vypočítat", command=vypocitej, width=12).pack(side="left", padx=(0, 8))
tk.Button(btns, text="Vyčistit", command=vycisti, width=10).pack(side="left", padx=(0, 8))
tk.Button(btns, text="Kopírovat", command=kopiruj, width=10).pack(side="left")

label_vysledek = tk.Label(frame, text="", justify="left", font=("Segoe UI", 10))
label_vysledek.grid(row=5, column=0, sticky="w", pady=(14, 0))

root.bind("<Return>", lambda e: vypocitej())
root.mainloop()